...
def choose_level():
  level = Tk()
  level.title("Choose Level")
  level.config(padx=50,pady=50,bg=BACKGROUND_COLOR)

  Label(level, text="Select Level", font=("Arial", 20, "bold"), bg=BACKGROUND_COLOR).pack(pady=20)

  Button(level, text="Easy", font=("Arial", 15), width=15, command=lambda: select(level,"Easy")).pack(pady=5)
  Button(level, text="Medium", font=("Arial", 15), width=15, command=lambda: select(level,"Medium")).pack(pady=5)
  Button(level, text="Hard", font=("Arial", 15), width=15, command=lambda: select(level,"Hard")).pack(pady=5)

  level.mainloop()

def select(window_to_close,level):
...
def start_game(level):
  global window, data, to_learn, flip_timer, canvas, main_k_word, k_heading, bg_image, front_card, back_card

    # Pick CSV file based on level
  if level == "Easy":
    path = "C:/tkinter projects/Word Guessing/CSV files/Kannada_to_English_Easy.csv"
  elif level == "Medium":
    path = "C:/tkinter projects/Word Guessing/CSV files/Kannada_to_English_Medium.csv"
  else:
    path = "C:/tkinter projects/Word Guessing/CSV files/Kannada_to_English_Hard.csv"
  ...
  window = Tk()
  window.title(f"Word Guess- {level} level ")
